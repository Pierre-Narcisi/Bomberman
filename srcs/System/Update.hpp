		...
		static void Bomb()
		{
			...
			for (auto &id : fl.list) {
				if (key[id].actions["attack"].state) {
					pos.Y = being[id]._node->getPosition().Z + 36 - std::fmod(static_cast<double>(being[id]._node->getPosition().Z + 36) , 100.0);
					pos.X = being[id]._node->getPosition().X + 55 - std::fmod(static_cast<double>(being[id]._node->getPosition().X + 55) , 100.0);
